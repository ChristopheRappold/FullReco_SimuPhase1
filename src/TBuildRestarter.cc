...
TBuildRestarter<Out>::TBuildRestarter(const THyphiAttributes& attribut) : TDataBuilder("build_restart"), att(attribut)
{
  att._logger->info("TBuildRestarter<Out>::TBuildRestarter");
}
template<typename Out>
TBuildRestarter<Out>::~TBuildRestarter() {}
#ifdef ROOT6
template<typename Out>
ReturnRes::InfoM TBuildRestarter<Out>::operator()(const Out& event, FullRecoEvent& RecoEvent,
                                             Out* OutTree)
{
...
#else
template<typename Out>
ReturnRes::InfoM TBuildRestarter<Out>::operator()(Out* event, FullRecoEvent& RecoEvent,
                                             Out* OutTree)
{
...
#endif
template<typename Out>
void TBuildRestarter<Out>::SelectHists() { LocalHisto.h_stats = AnaHisto->CloneAndRegister(AnaHisto->h_stats); }

template<typename Out>
ReturnRes::InfoM TBuildRestarter<Out>::SoftExit(int return_build)
{
  if(return_build == -1)
    {
...
#ifdef ROOT6
template<typename Out>
int TBuildRestarter<Out>::Exec(const Out& event, FullRecoEvent& RecoEvent, Out* OutTree)
#else
template<typename Out>
int TBuildRestarter<Out>::Exec(Out* event, FullRecoEvent& RecoEvent, Out* OutTree)
#endif
{
  int NumFilled = 0;
