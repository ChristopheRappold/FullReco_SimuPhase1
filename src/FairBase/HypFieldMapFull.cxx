...
#include "HypFieldMapFullData.h"

using namespace std;

// -------------   Default constructor  ----------------------------------
HypFieldMapFull::HypFieldMapFull() 
  : FairField(),
    fFileName(""),
    fScale(1.0),
    funit(1.0),
...
HypFieldMapFull::HypFieldMapFull(const char* mapName)//, const char* fileType)
  : FairField(mapName),
    fFileName(TString("")),
    fScale(1.0),
    funit(10.0), 
...
HypFieldMapFull::HypFieldMapFull(HypFieldPar* fieldPar) 
  : FairField(),
	fFileName(TString("")),
    fScale(1.0),
...
Double_t HypFieldMapFull::GetBx(Double_t x, Double_t y, Double_t z) 
{
  return 0.;
...
Double_t HypFieldMapFull::GetBy(Double_t x, Double_t y, Double_t z) {
...
Double_t HypFieldMapFull::GetBz(Double_t x, Double_t y, Double_t z) 
{
...
Bool_t HypFieldMapFull::IsInside(Double_t x, Double_t y, Double_t z,
...
void HypFieldMapFull::WriteAsciiFile(const char* fileName) {

  // Open file
  cout << "-I- HypFieldMapFull: Writing field map to ASCII file " 
       << fileName << endl;
  ofstream mapFile(fileName);
  if ( ! mapFile.is_open() ) {
    cerr << "-E- HypFieldMapFull:ReadAsciiFile: Could not open file! " << endl;
    return;
  }
  ...
  cout << "-I- HypFieldMapFull: " << fNx*fNy*fNz << " entries to write... " 
       << setw(3) << 0 << " % ";
  Int_t index=0;
  div_t modul;
...
void HypFieldMapFull::WriteRootFile(const char* fileName,
				const char* mapName) {

  HypFieldMapFullData* data = new HypFieldMapFullData(mapName, *this);
  TFile* oldFile = gFile;
...
void HypFieldMapFull::SetPosition(Double_t x, Double_t y, Double_t z,Double_t angle) {
  fPosX = x;
  fPosY = y;
  fPosZ = z;
  fAngleRot = angle;
...
void HypFieldMapFull::ReadAsciiFile(const char* fileName) {

  Double_t bx=0., by=0., bz=0.;
  Double_t xx, yy, zz;
  // Open file
  cout << "-I- HypFieldMapFull: Reading field map from ASCII file " 
       << fileName << endl;
  ...
  if ( fType != iType ) {
    cout << "-E- HypFieldMapFull::ReadAsciiFile: Incompatible map types!"
	 << endl;
  ...
  cout << right;
  Int_t nTot = fNx * fNy * fNz;
  cout << "-I- HypFieldMapFull: " << nTot << " entries to read... " 
       << setw(3) << 0 << " % ";
  Int_t index = 0;
  div_t modul;
  Int_t iDiv = TMath::Nint(nTot/100.);
  for (Int_t ix=0; ix<fNx; ix++) {
    for (Int_t iy = 0; iy<fNy; iy++) {
      for (Int_t iz = 0; iz<fNz; iz++) {
	if (! mapFile.good()) cerr << "-E- HypFieldMapFull::ReadAsciiFile: "
				   << "I/O Error at " << ix << " "
				   << iy << " " << iz << endl;
	index = ix*fNy*fNz + iy*fNz + iz;
	modul = div(index,iDiv);
	...
	if ( mapFile.eof() ) {
	  cerr << endl << "-E- HypFieldMapFull::ReadAsciiFile: EOF"
...
void HypFieldMapFull::ReadRootFile(const char* fileName, 
			       const char* mapName) {
  ...
  // Open root file
  cout << "-I- HypFieldMapFull: Reading field map from ROOT file " 
       << fileName << endl; 
  TFile* file = new TFile(fileName, "READ");		
  if (file->IsZombie()) {
    cerr << "-E- HypFieldMapFull::ReadRootfile: Cannot read from file! " 
	 << endl;
    Fatal("ReadRootFile","Cannot read from file");
  }

  // Get the field data object
  HypFieldMapFullData* data = NULL;
  file->GetObject(mapName, data);
  if ( ! data ) {
    cout << "-E- HypFieldMapFull::ReadRootFile: data object " << mapName
...
void HypFieldMapFull::SetField(const HypFieldMapFullData* data) {

  // Check compatibility
  if ( data->GetType() != fType ) {
    cout << "-E- HypFieldMapFull::SetField: Incompatible map types!"
	 << endl;
    cout << "    Field map is of type " << fType 
...
Double_t HypFieldMapFull::Interpolate(Double_t dx, Double_t dy, Double_t dz) {
...
void HypFieldMapFull::GetBxyz(const Double_t point[3], Double_t* bField)
{
    Int_t ix    = 0;
    Int_t iy    = 0;
    Int_t iz    = 0;
