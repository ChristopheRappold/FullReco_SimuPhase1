...
TKalmanFilter_DAF_ZMQ::TKalmanFilter_DAF_ZMQ(const THyphiAttributes& attribut) : TDataProcessInterface("mom_fit_kalman"), att(attribut)
{
...
TKalmanFilter_DAF_ZMQ::~TKalmanFilter_DAF_ZMQ()
{
  if(Vtracks != nullptr)
    {
      delete Vtracks;
...
void TKalmanFilter_DAF_ZMQ::InitMT()
{
  auto nav = gGeoManager->AddNavigator();
  //att._logger->debug("!> KF InitMT> added nav {}",fmt::ptr(nav));
  
}

ReturnRes::InfoM TKalmanFilter_DAF_ZMQ::operator()(DataBuilderOut& RecoEvent, DataFitterOut* Out)
{

  int result_mom = Exec(RecoEvent,Out);
...
void TKalmanFilter_DAF_ZMQ::SelectHists()
{
...
int TKalmanFilter_DAF_ZMQ::Exec(DataBuilderOut& RecoEvent, DataFitterOut* resOut)
{
  int res = Kalman_Filter_BuildData(RecoEvent);
  if(res != 0)
    return -1;
  ...
  if(result_kalman == 0)
    return Kalman_Filter_BuildOutput(RecoEvent, resOut);
  else
    return result_kalman;
}

ReturnRes::InfoM TKalmanFilter_DAF_ZMQ::SoftExit(int result_full)
{
  switch(result_full)
    {
    case -1:
      return ReturnRes::KalmanError;
    default:
      return ReturnRes::Fine;
    }
}

int TKalmanFilter_DAF_ZMQ::Kalman_Filter_BuildOutput(DataBuilderOut& RecoEvent, DataFitterOut* resOut)
{

  resOut->previousStep.status = RecoEvent.status;
  resOut->previousStep.idEvent = RecoEvent.idEvent;
  resOut->previousStep.DumpParticles = std::move(RecoEvent.DumpParticles);
  resOut->previousStep.DumpHits = std::move(RecoEvent.DumpHits);
  
  resOut->idEvent = currentEvent;
  
  for(auto TrackRes : DAF_results)
    {
      int TrackID = TrackRes.first;
      const ResSolDAF& FitRes = TrackRes.second;
      auto TInfo = TrackInfo.find(TrackID);
      auto IsDecay = TrackMother.find(TrackID);
      int Decay = IsDecay == TrackMother.end() ? 0 : 1;

      OutTrack& OutTrack = resOut->DumpTracks.emplace_back();
      ...
      OutTrack.MomMass = {FitRes.momX, FitRes.momY, FitRes.momZ, FitRes.mass};
      OutTrack.Mom = {FitRes.momX, FitRes.momY, FitRes.momZ};
      ...
      OutTrack.RefPoint = {FitRes.posX, FitRes.posY, FitRes.posZ};
      OutTrack.Pval2 = FitRes.pvalue;
      OutTrack.PathLength = FitRes.path_length;
      OutTrack.TOF = FitRes.tof;

      OutTrack.MomIni = {FitRes.momX_init, FitRes.momY_init, FitRes.momZ_init};
      ...
      if(Decay == 1)
        OutTrack.Sim2Vtx = {std::get<1>(IsDecay->second), std::get<2>(IsDecay->second), std::get<3>(IsDecay->second),
...
int TKalmanFilter_DAF_ZMQ::Kalman_Filter_BuildData(DataBuilderOut& RecoEvent)
{
  ListHits.clear();
  ...
  currentEvent = RecoEvent.idEvent;

  att._logger->info("Fitter> BuildData : size Particle{} Hit{}", RecoEvent.DumpParticles.size(), RecoEvent.DumpHits.size());

  
  for(size_t i=0;i<RecoEvent.DumpParticles.size();++i)
    {
      int TrackID = RecoEvent.DumpParticles[i].Mc_id;
     
      std::vector<int> tempSetHit(G4Sol::SIZEOF_G4SOLDETTYPE, -1);
      TrackDAF.insert(std::make_pair(TrackID, tempSetHit));
      
      std::vector<SimHit> tempSetSimHit(G4Sol::SIZEOF_G4SOLDETTYPE);
      TrackDAFSim.insert(std::make_pair(TrackID, tempSetSimHit));
      
      std::vector<InfoPar> tempSetInfo(G4Sol::SIZEOF_G4SOLDETTYPE);
      TrackInfo.insert(std::make_pair(TrackID, tempSetInfo));

      if(RecoEvent.DumpParticles[i].Mother_id!=-1)
	{
	  TrackMother.insert(
			     std::make_pair(TrackID, std::make_tuple(RecoEvent.DumpParticles[i].Mother_id,
								     RecoEvent.DumpParticles[i].Vtx[0],RecoEvent.DumpParticles[i].Vtx[1],
								     RecoEvent.DumpParticles[i].Vtx[2],RecoEvent.DumpParticles[i].Vtx[3])
					    )
			     );
  ...
  ListHits.resize(G4Sol::SIZEOF_G4SOLDETTYPE);
  ...
  for(int TypeDet = G4Sol::InSi0, id_det_last = G4Sol::FMF2Stop2; TypeDet <= id_det_last; ++TypeDet)
    {
      ...
      auto& tempHits = RecoEvent.DumpHits[TypeDet];
      
      for(auto it_hit = tempHits.cbegin(), it_hit_end = tempHits.cend(); it_hit != it_hit_end; ++it_hit)
	{
	  auto& hit          = *it_hit;
	  int indexInBranch = std::distance(tempHits.cbegin(), it_hit);

	  int TrackID = hit.MC_id;
	  int LayerID = hit.LayerID;

	  auto tempTrack = TrackDAF.find(TrackID);
	  if(tempTrack == TrackDAF.end())
	    continue;

	  TVectorD hitCoordsTree(3);
	  if(ZMQ::G4Sol::IsPlanar(G4Sol::SolDet(TypeDet)))
	    {
	      TVectorD hitCoords(2);
	      hitCoords(0) = hit.Hit[0];
	      hitCoords(1) = hit.Hit[1];
		...
		std::make_unique<genfit::PlanarMeasurement>(hitCoords, hitCov, int(TypeDet), LayerID, nullptr);
	      ...
	      TVector3 o(0., 0., hit.Hit[2]), u(1., 0., 0.), v(0., 1., 0.);
	  ...
	  ListHits[TypeDet].emplace_back(measurement.release());
	  int indexHit = ListHits[TypeDet].size() - 1;
	  
	  tempTrack->second[TypeDet] = indexHit;
	  
	  int pdg_code = hit.Pdg;
	  
	  auto tempTrackSim                     = TrackDAFSim.find(TrackID);
	  tempTrackSim->second[TypeDet].layerID = LayerID;
	  tempTrackSim->second[TypeDet].hitX    = hit.MCHit[0];
	  tempTrackSim->second[TypeDet].hitY    = hit.MCHit[1];
	  tempTrackSim->second[TypeDet].hitZ    = hit.MCHit[2];
	  tempTrackSim->second[TypeDet].momX    = hit.MCparticle[0];
	  tempTrackSim->second[TypeDet].momY	= hit.MCparticle[1];
	  tempTrackSim->second[TypeDet].momZ	= hit.MCparticle[2];
	  tempTrackSim->second[TypeDet].pdg     = pdg_code;
	  tempTrackSim->second[TypeDet].mass    = hit.MCparticle[3];
	  tempTrackSim->second[TypeDet].Eloss   = hit.Energy;
	  tempTrackSim->second[TypeDet].time    = gRandom->Gaus(hit.Time, time_res);
	  tempTrackSim->second[TypeDet].length  = hit.Length;
	  
	  auto tempTrackInfo                    = TrackInfo.find(TrackID);
	  tempTrackInfo->second[TypeDet].pdg    = pdg_code;
	  tempTrackInfo->second[TypeDet].momX   = hit.MCparticle[0];
	  tempTrackInfo->second[TypeDet].momY   = hit.MCparticle[1];
	  tempTrackInfo->second[TypeDet].momZ   = hit.MCparticle[2];
	  tempTrackInfo->second[TypeDet].mass   = hit.MCparticle[3];
	  tempTrackInfo->second[TypeDet].Eloss  = hit.Energy;
	  tempTrackInfo->second[TypeDet].time   = hit.Time;
	  tempTrackInfo->second[TypeDet].length = hit.Length;
	  
	  const double charge = hit.Charge;
	}
    }
...
int TKalmanFilter_DAF_ZMQ::Kalman_Filter_FromTrack(DataBuilderOut& RecoEvent)
{
  // TGeoNavigatorArray* list_nav = gGeoManager->GetListOfNavigators();
  // att._logger->debug("!> KF> list nav :{}",fmt::ptr(list_nav));
  // if(list_nav!=nullptr)
  //   for(int i=0; i < list_nav->GetEntries();++i)
  //     att._logger->debug("!> KF> nav#{} {}",i, fmt::ptr(list_nav->At(i)));

  gGeoManager->SetCurrentNavigator(0);
  std::stringstream ss1;
  ss1 <<  std::this_thread::get_id() ;
  
  att._logger->debug("! KF> in kalman : {}",ss1.str());
  ...
  unsigned int Nb_trackSeed = TrackDAF.size();

  int ntrack = -1;
  for(auto it_trackInfo : TrackInfo)
    {
      ++ntrack;
...
// std::vector<std::string> name_det;
      ...
      auto it_ListHits = TrackDAF.find(id_track);
	  ...
	  if(id_det>=G4Sol::MG01 && id_det<=G4Sol::MG17)
	    ++n_Central;
          genfit::AbsMeasurement* currentHit = ListHits[id_det][id_hit].get();
      ...
      if(n_Central<9)
	{
      ...
      genfit::AbsMeasurement* tempHit = ListHits[id_firstDet][std::get<2>(*firstHit)].get();
	...
	att._logger->debug( "det:{} [{}] at Z:{}", std::get<1>(idet), std::get<2>(idet), std::get<0>(idet) );
      ...
      genfit::AbsMeasurement* FirstHit = ListHits[id_lastDet][std::get<2>(*lastHit)].get();
      TVectorD& HitrawRef = FirstHit->getRawHitCoords();
...
//   }
	  ...
	  tempResults.dE = dE;
          ...
          DAF_results.insert(std::make_pair(id_track, tempResults));
