...
#ifdef ROOT6
int TBuildDetectorLayerPlaneDAF::operator() (const TG4Sol_Event& event, const std::vector<TTreeReaderArray<TG4Sol_Hit>*>& hits, FullRecoEvent& RecoEvent, MCAnaEventG4Sol* OutTree)
{
...
#else
int TBuildDetectorLayerPlaneDAF::operator() (const TG4Sol_Event& event, const std::vector<TClonesArray*>& hits, FullRecoEvent& RecoEvent, MCAnaEventG4Sol* OutTree)
{
  int result = Exec(event,hits,RecoEvent, OutTree);
...
#ifdef ROOT6
int TBuildDetectorLayerPlaneDAF::Exec(const TG4Sol_Event& event, const std::vector<TTreeReaderArray<TG4Sol_Hit>*>& hits, FullRecoEvent& RecoEvent, MCAnaEventG4Sol* OutTree)
#else
int TBuildDetectorLayerPlaneDAF::Exec(const TG4Sol_Event& event, const std::vector<TClonesArray*>& hits, FullRecoEvent& RecoEvent, MCAnaEventG4Sol* OutTree)
#endif
{
  int NumFilled=0;
  ...
  for(size_t id = 0;id<event.BeamNames.size();++id)
      ...
      for(auto& det : hits)
	{
	  ...
	  for(auto hit : *det)
		...
		std::cout<<"Branch: "<<det->GetBranchName()<<" "<<hit.HitPosX<<" "<<hit.HitPosY<<" "<<hit.HitPosZ<<" "<<" "<<hit.LayerID<<" "<<hit.Pname<<"\n";
	    }
#else
	  for(int j=0;j<det->GetEntries();++j)
	    {
	      ...
	      if(hit->TrackID==trackID)
		  std::cout<<"Branch: "<<det->GetName()<<" hit:"<<hit->HitPosX<<" "<<hit->HitPosY<<" "<<hit->HitPosZ<<" mom:"<<hit->MomX<<" "<<hit->MomY<<" "<<hit->MomZ<<" | "<<" "<<hit->LayerID<<" "<<hit->Pname<<"\n";
	    }
#endif	  
	}
    }
  ...
  for(size_t index = 0; index < event.BeamTrackID.size();++index)
    {
      ...
      OutParticle->MomMass.SetXYZM(event.BeamMomentums_X[index],event.BeamMomentums_Y[index],event.BeamMomentums_Z[index],event.BeamMasses[index]);
  ...
  for(size_t index = 0; index < event.DaughterTrackID.size();++index)
    {
      ...
      OutParticle->MomMass.SetXYZM(event.DaughterMomentums_X[index],event.DaughterMomentums_Y[index],event.DaughterMomentums_Z[index],event.DaughterMasses[index]);
      ...
      if(nameTempBr=="FMF2_log" || nameTempBr=="HypHI_TrackFwd_log")
	{
	  ...
	  for( auto it_hit = tempHits->begin(), it_hit_end = tempHits->end();it_hit!=it_hit_end;++it_hit)
		...
		continue;
	      ...
	      tempTrackSim->second[TypeDet+LayerID].time=hit.Time;
