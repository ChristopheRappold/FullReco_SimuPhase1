...
using TDataProcessInterface = TDataProcess<FullRecoEvent, int>;
...
class TKalmanFilter_DAF_MT : public TDataProcessInterface
{
  ...
  explicit TKalmanFilter_DAF_MT(const THyphiAttributes& attr);
  ~TKalmanFilter_DAF_MT() final;

  // int Init(Ana_Hist* h);
  ReturnRes::InfoM operator()(FullRecoEvent& RecoEvent, int*) final;
  void InitMT() final;
  
  private:
  int Exec(FullRecoEvent& RecoEvent, int*) final;
