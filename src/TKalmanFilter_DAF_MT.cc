...
TKalmanFilter_DAF_MT::TKalmanFilter_DAF_MT(const THyphiAttributes& attribut) : TDataProcessInterface("mom_fit_kalman"), att(attribut)
{
...
TKalmanFilter_DAF_MT::~TKalmanFilter_DAF_MT()
{
  if(Vtracks != nullptr)
    {
      delete Vtracks;
...
void TKalmanFilter_DAF_MT::InitMT()
{
  auto nav = gGeoManager->AddNavigator();
  att._logger->debug("!> KF InitMT> added nav {}",fmt::ptr(nav));
}

ReturnRes::InfoM TKalmanFilter_DAF_MT::operator()(FullRecoEvent& RecoEvent, int*)
{

  int result_mom = Exec(RecoEvent,nullptr);
...
void TKalmanFilter_DAF_MT::SelectHists()
{
...
int TKalmanFilter_DAF_MT::Exec(FullRecoEvent& RecoEvent, int*)
{
