...
TrackPoint::TrackPoint() :
  OwnMeasurements(true),sortingParameter_(0), track_(NULL), thinScatterer_(NULL)
{
  ;
}

TrackPoint::TrackPoint(Track* track) :
  OwnMeasurements(true),sortingParameter_(0), track_(track), thinScatterer_(NULL)
{
  ;
}

  TrackPoint::TrackPoint(const std::vector< genfit::AbsMeasurement* >& rawMeasurements, Track* track, bool Owner_) :
  OwnMeasurements(Owner_),sortingParameter_(0), track_(track), thinScatterer_(NULL)
{
...
TrackPoint::TrackPoint(const TrackPoint& rhs) :
  TObject(rhs),
  OwnMeasurements(rhs.OwnMeasurements),sortingParameter_(rhs.sortingParameter_), track_(rhs.track_), thinScatterer_(NULL)
{
