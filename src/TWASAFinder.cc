    ...
    for(int i=0; i<RecoEvent.FiberTrackCont[track_type].size(); ++i){
      ...
      //track_hit->SetDidCh(); //CHECK Maybe include later
        ...
        for( auto v : RecoEvent.FiberTrackCont[track_type][i]->GetContHit() ){
          ...
          //printf("det: %d , lay: %d, tot: %d, fib: %d \n", v->GetDet(), v->GetLay(), (v->GetDet()-3)*3 + v->GetLay(), v->GetFib());
        }
      ...
      //std::cout << "num before : " << track_hit->GetNum() << std::endl;
      //track_hit->Print();
      //std::cout << "num after  : " << track_hit->GetNum() << std::endl;
      //track_hit->Print();
      ...
      for(int j = 0; j < 6; ++j)
        {
          ...
          for(int k = 0; k < (int)RecoEvent.ListHits[G4Sol::MiniFiberD1_x1 + j].size(); ++k)
            {
              //printf("TrackHit: %d ; ListHists: %d \n", TrackHitCont[i]->GetFiberHit()[j], RecoEvent.ListHits[G4Sol::MiniFiberD1_x1 + j][k]->getHitId());
              if(TrackHitCont[i]->GetFiberHit()[j] == RecoEvent.ListHits[G4Sol::MiniFiberD1_x1 + j][k]->getHitId())
                {
                  continue; //CHECK Remove when FiberID is same for data and simu
                  tempSetHit[G4Sol::MiniFiberD1_x1 + j] = k;
                  InfoPar tmp_infopar;
                  tmp_infopar.pdg    = -211;
                  ...
                  tempSetInfo[G4Sol::MiniFiberD1_x1 + j] = tmp_infopar;
                  flag_found = true;
                }
            }
