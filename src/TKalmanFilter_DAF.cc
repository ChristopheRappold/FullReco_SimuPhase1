...
TKalmanFilter_DAF::TKalmanFilter_DAF(const THyphiAttributes& attribut):TDataProcessInterface("mom_fit_kalman"),att(attribut)
{
  ...
  Fitter = new genfit::KalmanFitter(6,dPVal,1e-3,true);//new genfit::DAF(true);
...
int TKalmanFilter_DAF::SoftExit(int result_full)
{
  return result_full;    
}
  ...
  for( auto it_trackInfo : RecoEvent.TrackInfo)
    {
      ...
      //std::cout<<" | Pointsize "<<Vtracks->getNumPoints()<<" Measurements "<<Vtracks->getNumPointsWithMeasurement()<<" Rep "<<Vtracks->getNumReps()<<std::endl;
	...
	continue;
      ...
      const InfoPar track_state(it_trackInfo.second[id_firstHit]);
...
#ifdef DEBUG_KALMAN
      std::cout<<"init mom : "; init_p.Print();
#endif
      ...
      if(fitTrack->getFitStatus(REP)->isFitConverged())
	{
	  ...
	  const double chi2= whoDoFit == 1 ? Fitter->getChiSqu(fitTrack.get(),REP) : Fitter_rescue->getChiSqu(fitTrack.get(),REP); //Vtracks->getChiSqu();

	  const double ndf= whoDoFit == 1 ? Fitter->getNdf(fitTrack.get(),REP) : Fitter_rescue->getNdf(fitTrack.get(),REP);//static_cast<int>(Vtracks->getNDF());
	  ...
	  const double p_value = whoDoFit == 1 ? Fitter->getPVal(fitTrack.get(),REP) : Fitter_rescue->getPVal(fitTrack.get(),REP);//TMath::Prob(chi2,ndf);
	  ...
	  std::cout<<"TEveLine* EveTrack_DAF1 = new TEveLine(TrackRepList_DAF,"<<rep_length->EveListTrackPoint.size()<<");"<<std::endl;
	  for(unsigned int i = 0;i<rep_length->EveListTrackPoint.size();++i)
	    {
	      std::cout<<std::setprecision(10)<<"EveTrack_DAF1->SetPoint("<<i<<","<<rep_length->EveListTrackPoint[i].X()<<","<<rep_length->EveListTrackPoint[i].Y()<<","<<rep_length->EveListTrackPoint[i].Z()<<");"<<std::endl;
	    }	
	  ...
	  const double beta = 1./30.*Path_lengthMean/time_of_flight; ///beta = dL[m]/(c[m/s]* dT[s]) = dL[mm]/(300[mm/ns] * dT[ns])
	  ...
	  if(beta < 0.)
	    {
	      cout<<"E> Beta Negative ! "<<Path_lengthMean<<" "<<time_of_flight<<" | "<<Path_length<<" "<<endl;//<<Path_lengthB<<" "<<endl;
	      cout<<charge<<" "<<p<<" "<<chi2<<" "<<ndf<<" "<<p_value2<<endl;
	    }
 
#ifdef DEBUG_KALMAN2
	  std::cout<<"charge: "<<charge<<" / time = "<<time_of_flight /*Detectors_UTr["TOF"][id_track][3]*/ <<" / Path = "<<Path_length<<" PathMean = "<<Path_lengthMean  <<" / Beta = "<<beta<<" / Mass = "<<mass<<std::endl;
#endif
	  ...
	  tempResults.firstHit = id_firstHit;
